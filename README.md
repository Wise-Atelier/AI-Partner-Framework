...
*(Source-available Project / Code: PolyForm Noncommercial 1.0.0, Docs: CC BY-NC-SA 4.0)*
...
- 私は収益を得ず、オープンソース的な精神と同人文化を融合させる  

ソース可用な公開によって文化の共有を進め、  
...
- **回答生成**  
  本フレームワークでは、日本語能力に優れたローカル LLM を「言語野」として利用する。
  応答の際には記憶・感情モデル、人格プロファイル、ファインチューニング済みスタイルを条件として与え、
  単なる質問応答ではなく、感情や文脈を反映した自然で一貫性のある対話を行う。 

- **記憶と感情モデル**  
  プルチックの「感情の輪」に基づき、喜び・信頼・恐怖・驚き・悲しみ・嫌悪・怒り・期待をパラメータとして管理する。
  感情は会話や出来事によって増減し、キャラクターの態度や言い回しに影響を与える。
  さらに孤独感・好奇心などの内的ドライブ変数を保持し、キャラクターごとに調整可能な気質差を表現する。
...
- **自発的発話**  
  ユーザーから入力がなくても、一定時間の経過や「未解決の話題」などを契機として自ら発話できる。
  記憶に残る出来事や感情を題材とした独り言・呼びかけにより、関係性の自然な深化を実現する。
  マルチモーダル化後は、ユーザーがカメラの視界に入ったときや表情を検知したときに挨拶や共感を示すなど、状況依存の自発行動へ拡張される。

- **人格・性格ファインチューニング**  
  本フレームワークは、対話データを用いた人格・性格のファインチューニングに対応可能な設計を持つ。
  ただし本リポジトリには学習済みモデルやデータは含まれず、利用者が独自にデータを用意する必要がある。
  この仕組みにより、利用者はキャラクターの個性や関係性に合わせて調整し、
  「推しらしさ」や「自分だけのパートナー性格」 を再現できる柔軟性を確保する。
...
## ライセンス方針
- **コード部分**: PolyForm Noncommercial 1.0.0（非商用に限り利用・改変・配布可）  
- **文書・設計思想・構想部分**: CC BY-NC-SA 4.0  
- **音声データ / 3D モデル / ファインチューニング用データ**: 本リポジトリには含まれません。  
  → 利用者は必ず各権利者と正規契約を行ってください。  
- 本プロジェクトは **非商用条件**を含むため、OSI の「オープンソース」定義とは異なります。配布形態としては **ソース可用（source-available）** に分類されます。
...
## 倫理的スタンス（推奨）
- 本フレームワークの思想として、**未成年を対象としたロマンティック／性的な振る舞いの模倣・助長は推奨しません**。  
- 年齢確認や機能ロックは**推奨機能**として設計に含めますが、**最終的な利用方法と責任はユーザーに委ねられます**。  
...
- ※ 追記: [セキュリティとプライバシー設計](/docs/07_security_privacy.md)  
- ※ 追記: [プラグイン API](/docs/08_plugin_api.md)  
- ※ 追記: [リモートモード（Self-hosting / WebRTC）](/docs/09_remote_mode.md)  
- ※ 追記: [倫理ガイド](/docs/10_ethics_safety.md)  
- ※ 追記: [法務補足 & 用語FAQ](/docs/11_legal_compliance.md)  
- ※ 追記: [記憶設計の具体式](/docs/12_memory_design.md)  
